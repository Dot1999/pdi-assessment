        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="flex justify-between p-3">
                    ...
                    <button data-modal-backdrop="static" data-modal-url="{{route('user.create')}}" type="button" class="text-white bg-blue-700 hover:bg-blue-800 text-lg focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-lg px-6 py-2 text-center light:bg-blue-600 light:hover:bg-blue-700 light:focus:ring-blue-800">Add Employee</button>
                    @endif
                    <div class="flex align-center">
                        <button onclick="print('{{route('user.print')}}/?search_query={{request('search')}}')" type="button" class="text-white bg-blue-700 hover:bg-blue-800 text-lg focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-lg px-6 py-2 text-center light:bg-blue-600 light:hover:bg-blue-700 light:focus:ring-blue-800">Print</button>
                        @if (auth()->user()->canAccess('user-export'))
                        <form action="{{route('user.export')}}/?search_query={{request('search')}}" method="POST" class="flex px-2">
                            @csrf
                            <div class="flex">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 light:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 light:bg-gray-700 light:text-gray-400">
                            <tr>
                                ...
                                @if (auth()->user()->canAccess('user-update') || auth()->user()->canAccess('user-delete'))
                                <th scope="col" class="px-6 py-3">
                                    Action
                                </th>
                                @endif
                        ...
                        <tbody>
                            ...
                            <tr class="bg-white border-b light:bg-gray-800 light:border-gray-700 hover:bg-gray-50 light:hover:bg-gray-600">
                                ...
                                <td class="flex items-center px-6 py-4 space-x-3">
                                    @if (auth()->user()->canAccess('user-view'))<button type="button" data-modal-url="{{route('user.show', ['user' => $user->id])}}" class="text-white bg-yellow-400 hover:bg-yellow-500 focus:outline-none focus:ring-4 focus:ring-yellow-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 light:focus:ring-yellow-900">Permissions</button>@endif
                                    @if (auth()->user()->canAccess('user-update'))<button type="button" data-modal-url="{{route('user.edit', ['user' => $user->id])}}" class="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-full text-sm px-5 py-2.5 mr-2 mb-2 light:bg-gray-800 light:text-white light:border-gray-600 light:hover:bg-gray-700 light:hover:border-gray-600 light:focus:ring-gray-700">Edit</button>@endif
                                    @if (auth()->user()->canAccess('user-delete'))<button type="button" data-modal-url="{{route('user.delete-modal', ['id' => $user->id])}}" class="text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 light:bg-red-600 light:hover:bg-red-700 light:focus:ring-red-900">Delete</button>@endif
                                </td>
                                @endif
                            </tr>
                            @empty
                            <tr>
                                <td class="text-center p-5 text-black" colspan="{{(auth()->user()->canAccess('user-update') || auth()->user()->canAccess('user-delete')) ? 5 : 4}}">No data found</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>

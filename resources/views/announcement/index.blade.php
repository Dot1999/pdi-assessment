<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="flex justify-between p-3">
                    ...
                    <button data-script="initAnnouncementCreation" data-modal-backdrop="static" data-modal-url="{{route('announcement.create')}}" type="button" class="text-white bg-blue-700 hover:bg-blue-800 text-lg focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-lg px-6 py-2 text-center light:bg-blue-600 light:hover:bg-blue-700 light:focus:ring-blue-800">Add Announcement</button>
                    @endif
                    <div class="flex align-center">
                        <button onclick="print('{{route('announcement.print')}}/?search_query={{request('search')}}')" type="button" class="text-white bg-blue-700 hover:bg-blue-800 text-lg focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-lg px-6 py-2 text-center light:bg-blue-600 light:hover:bg-blue-700 light:focus:ring-blue-800">Print</button>
                        @if (auth()->user()->canAccess('announcement-export'))
                        <form action="{{route('announcement.export')}}/?search_query={{request('search')}}" method="POST" class="flex px-2">
                            @csrf
                            <div class="flex">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 light:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 light:bg-gray-700 light:text-gray-400">
                            <tr>
                                ...
                                @if (auth()->user()->canAccess('announcement-update') || auth()->user()->canAccess('announcement-delete'))
                                <th scope="col" class="px-6 py-3">
                                    Action
                                </th>
                                @endif
    ...
    <script>
        function initAnnouncementCreation() {
            const departmentCheckboxesDiv = document.querySelector("#department-checkboxes");
            const checkboxes = departmentCheckboxesDiv.querySelectorAll('.department-check');

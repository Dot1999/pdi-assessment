<div class="relative w-full h-full max-w-2xl md:h-auto">
    <form action="{{route('announcement.update', ['announcement' => $announcement->id])}}" method="post">
        @method('PUT')
        @csrf
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow light:bg-gray-700">
            ...
            <div class="p-6 space-y-6">
                <div class="mb-6">
                    ...
                    <input type="text" name="title" id="title" value="{{$announcement->title}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 light:bg-gray-700 light:border-gray-600 light:placeholder-gray-400 light:text-white light:focus:ring-blue-500 light:focus:border-blue-500" required>
                </div>
                <div class="mb-6">
                    <label for="content" class="block mb-2 text-sm font-medium text-gray-900 light:text-white">Content</label>
                    <textarea id="content" name="content" rows="5" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 light:bg-gray-700 light:border-gray-600 light:placeholder-gray-400 light:text-white light:focus:ring-blue-500 light:focus:border-blue-500" required placeholder="Write your announcement here...">{{$announcement->content}}</textarea>
                </div>
                <div class="mb-6">
                    <h6>Departments</h6>
                    <div class="flex" id="department-checkboxes">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            @php
                                $department_ids = $announcement->department->pluck('id')->toArray();
                            @endphp
                            @foreach ($departments as $department)
                            <div class="flex items-center mr-4">
                                <input {{in_array($department->id,$department_ids) ? "checked":""}} name="department_id[]" id="checkbox-{{$department->name}}" type="checkbox" value="{{$department->id}}" class="department-check w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 light:focus:ring-blue-600 light:ring-offset-gray-800 focus:ring-2 light:bg-gray-700 light:border-gray-600">
